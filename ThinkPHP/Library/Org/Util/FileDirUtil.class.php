...
class FileDirUtil
{
     ...
     */
    public function dirIconv($in_charset, $out_charset, $dir, $fileexts = 'php|html|htm|shtml|shtm|js|txt|xml')
    {
     ...
     */
    public function dirTouch($path, $mtime = TIME, $atime = TIME)
    {
        if (!is_dir($path)) {
            return false;
        }
     ...
     */
    public function dirTree($dir, $parentid = 0, $dirs = array())
    {
        global $id;
        if ($parentid == 0) {
            $id = 0;
    ...
    @return array   返回目录列表
     */
    public function dirNodeTree($dir)
    {
        $d    = dir($dir);
        $dirs = array();
