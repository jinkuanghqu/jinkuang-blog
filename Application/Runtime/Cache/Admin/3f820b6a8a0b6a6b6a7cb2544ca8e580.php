...
<body class="iframe-body" data-sid="<?php echo ($adminRow["id"]); ?>" data-settime="<?php echo (C("IFRAME_BODY_SETTIME")); ?>" data-actionurl="/Admin/Seo/index">
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-collapse collapse in">
            <div class="panel-body">
                <div><a href="<?php echo U('/Admin/Seo/add');?>" class="btn btn-success" style="margin-left:5px">添加</a></div>

                <table class="table table-hover" style="margin-left:5px !important">
                    <thead>
                        <tr>
                            <th>序号</th><th>名称</th><th>页面路径</th>
                            <th>SEO标题</th>
                            <th>操作</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr>
                                <td><?php echo ($i); ?></td><td><?php echo ($item['name']); ?></td><td><?php echo ($item['page_path']); ?></td>
                                <td width="20%"><?php echo ($item['title']); ?></td>
                                <td>
                                     <div class="left">
                                        <span><a class="btn btn-sm btn-primary" href="<?php echo U('/Admin/Seo/edit', array('id'=>$item['id']));?>">编辑</a></span>
...
function drop(seoId){
    parent.layer.confirm( '您确定要删除这条数据吗？', {
        ...
        $.get("<?php echo U('/Admin/Seo/destroy');?>", {id:seoId}, AjaxRequest, 'json' );
