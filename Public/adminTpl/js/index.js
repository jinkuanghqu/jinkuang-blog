$( function() {
	...
	$( "#sidebar" ).on( 'click', 'li.nav-item', function( event ) {
		...
		if ( submenu ) {
			...
			$( '.product-nav-title' ).html( $this.text() );
			...
			if ( submLi.length > 0 ) {
				...
				var iframeSrc = submLi.eq( 0 ).find( '.ng-isolate-scope' ).addClass(
					'active' ).find( 'a' ).attr( "data-url" );
	...
	var sidem = $( "#sidebal-middle" ),
		mainFrame = $( "#mainFrame" );

	$( "#product-nav-list" ).on( 'click', 'a', function( event ) {
		var $this = $( this ),
			url = $this.attr( "data-url" );
		$this.parents( ".ng-isolate-scope" ).addClass( 'active' ).parents( "li" )
			.siblings( 'li' ).find( '.ng-isolate-scope' ).removeClass( 'active' );
		mainFrame.attr( "src", url );
	} );
